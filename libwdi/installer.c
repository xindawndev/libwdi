...
int __cdecl main(int argc_ansi, char** argv_ansi)
{
...
#if PRINTLOG
	{
		...
		char  phonePath[MAX_PATH] = { 0 };


		GetTempPath(MAX_PATH, tmpPath);

		int len = WideCharToMultiByte(CP_UTF8, 0, (const wchar_t*)tmpPath, -1, NULL, 0, NULL, NULL);
		WideCharToMultiByte(CP_UTF8, 0, (const wchar_t*)tmpPath, -1, tmpFileName, len, NULL, NULL);
		sprintf_s(phonePath, MAX_PATH, "%s%s", tmpFileName, "\\xindawn_install.log");
		fopen_s(&fhandle, phonePath, "a+");
		...
		if (fhandle)
		{
			...
			strftime(ch, sizeof(ch) - 1, "%Y-%m-%d %H:%M:%S", &now_time);
			fprintf(fhandle, "%s  %d\n", ch, __LINE__);
			fclose(fhandle);
		}
	}
	...
	if (hardware_id != NULL) {
...
#if PRINTLOG
		{
			...
			if (fhandle)
			{
				...
				localtime_s(&now_time, &t);
				strftime(ch, sizeof(ch) - 1, "%Y-%m-%d %H:%M:%S", &now_time);
				fprintf(fhandle, "%s  %d hardware_id = %s  path = %s\n", ch, __LINE__, hardware_id, path);
				fclose(fhandle);
			}
		}
#endif
