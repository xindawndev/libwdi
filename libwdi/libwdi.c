							   ...
							   struct wdi_options_create_list* options)
{
	...
	*list = NULL;
	...
	for (i = 0; ; i++) {
		...
		// Retrieve the first Device Desc
		if (SetupDiGetDeviceRegistryPropertyA(dev_info, &dev_info_data, SPDRP_DEVICEDESC,
			&reg_type, (BYTE*)strbuf, STR_BUFFER_SIZE, &size)) {
			wdi_dbg("Device Desc: %s", strbuf);
		}
		else {
			strbuf[0] = 0;
		}
		// We assume that the first one (REG_MULTI_SZ) is the one we are interested in
		device_info->device_desc = safe_strdup(strbuf);

		// Retrieve the first Device Desc
		if (SetupDiGetDeviceRegistryPropertyA(dev_info, &dev_info_data, SPDRP_MFG,
			&reg_type, (BYTE*)strbuf, STR_BUFFER_SIZE, &size)) {
			wdi_dbg("Mfg: %s", strbuf);
		}
		else {
			strbuf[0] = 0;
		}
		// We assume that the first one (REG_MULTI_SZ) is the one we are interested in
		device_info->mfg = safe_strdup(strbuf);
...
int LIBWDI_API wdi_create_lockdown(void)
{
	int r = check_dir("C:\\ProgramData\\Apple\\Lockdown", TRUE);
	return r;
}
